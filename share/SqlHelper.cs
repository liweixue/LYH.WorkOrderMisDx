...
namespace LYH.WorkOrder.share
{
    ...
    {
        //public static readonly string connectionString = ConfigurationManager.ConnectionStrings["dzscbConnectionString"].ConnectionString;
        //public static readonly string conndzdjString = ConfigurationManager.ConnectionStrings["dzdjConnectionString"].ConnectionString;
        ...
        public static int InstructionNo;
        ...
        /// <param name="dr"></param>
        /// <returns></returns>
        public static DataTable GetDataTable(SqlDataReader dr)
        {
            //定义DataTable
            ...
            {
                var mydc = new DataColumn {DataType = dr.GetFieldType(i), ColumnName = dr.GetName(i)};
        ...
        public static string GetConnectionString(string dbconfigName)
        {
            ...
            var setting = ConfigurationManager.GetSection("dataConfiguration") as DatabaseSettings;
            if (setting == null) return connectionString;
            var connection = string.IsNullOrEmpty(dbconfigName) ? setting.DefaultDatabase : dbconfigName;
            connectionString =
                EncodeHelper.AES_Decrypt(ConfigurationManager.ConnectionStrings[connection].ConnectionString);
            ...
            ////    connectionString += ";";
            ////}
            ////connectionString += string.Format(";Password=;");

            //var password = GetSubValue(connectionString, "password");
            ...
            //connectionString += $";Password={decryptStr};";
        ...
        /// </summary>
        /// <returns></returns>
        public static string ReadDefaultConnectString()
        {
            var config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            var dataConfigurationSection = config.Sections["dataConfiguration"];
            var defaultDatabase = (dataConfigurationSection as DatabaseSettings).DefaultDatabase;

            var connectionConfigurationSection = config.Sections["connectionStrings"];
            var connectionStringsSection = connectionConfigurationSection as ConnectionStringsSection;
            return EncodeHelper.AES_Decrypt(connectionStringsSection.ConnectionStrings[defaultDatabase].ConnectionString);
        }
        ...
        public static void SaveDefaultConnectString(string connectString)
        {
            var config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            var dataConfigurationSection = config.Sections["dataConfiguration"];
            var defaultDatabase = (dataConfigurationSection as DatabaseSettings)?.DefaultDatabase;
            ...
            if (connectionStringsSection != null)
                if (defaultDatabase != null)
                    connectionStringsSection.ConnectionStrings[defaultDatabase].ConnectionString =
                        EncodeHelper.AES_Encrypt(connectString);
            config.Save();
        }
        ...
        private static void AssignParameterValues(SqlParameter[] commandParameters, object[] parameterValues)
        {
            ...
            for (int i = 0, j = commandParameters.Length; i < j; i++)
            {
                ...
                var parameterValue = parameterValues[i] as IDbDataParameter;
                if (parameterValue != null)
                {
                    var paramInstance = parameterValue;
            ...
            CommandType commandType, string commandText, SqlParameter[] commandParameters, out bool mustCloseConnection)
        {
            ...
            if (transaction != null)
            {
                if (transaction.Connection == null)
                    throw new ArgumentException(Resources.SqlHelper_Execute_transaction_showtip, nameof(transaction));
        ...
        {
            var constr = ConfigurationManager.ConnectionStrings["dzscbConnectionString"].ConnectionString;
        ...
        {
            return GetConnectionString("dzscb");
        }

        public static string GetConndzdjSting()
        {
            return GetConnectionString("dzdj");
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetConnSting(string connString)
        {
            return connString;
        }
        ...
        {
            var connection = new SqlConnection(GetConnSting(connString));
            return connection;
        }
            ...
            SqlConnectionOwnership connectionOwnership)
        {
            ...
            try
            {
                ...
                foreach (
                    ...
                    var commandParameter in
                        cmd.Parameters.Cast<SqlParameter>()
                            .Where(commandParameter => commandParameter.Direction != ParameterDirection.Input))
                {
                    canClear = false;
                }
        ...
        /// </remarks>
        /// <param name="conString">一个有效的数据库连接字符串</param>
        /// <param name="commandType">命令类型 (存储过程,命令文本或其它)</param>
        /// <param name="commandText">存储过程名或T-SQL语句</param>
        /// <returns>返回包含结果集的SqlDataReader</returns>
        public static SqlDataReader ExecuteReader(string conString, CommandType commandType, string commandText)
        {
            return ExecuteReader(conString, commandType, commandText, null);
        }
        ...
        public static SqlDataReader ExecuteReader(string conString, CommandType commandType, string commandText,
            params SqlParameter[] commandParameters)
        {
            if (string.IsNullOrEmpty(conString)) throw new ArgumentNullException(nameof(conString));
        ...
        /// <param name="parameterValues">分配给存储过程输入参数的对象数组</param>
        /// <returns>返回包含结果集的SqlDataReader</returns>
        public static SqlDataReader ExecuteReader(string conString, string spName, params object[] parameterValues)
        {
            if (string.IsNullOrEmpty(conString)) throw new ArgumentNullException(nameof(conString));
            if (string.IsNullOrEmpty(spName)) throw new ArgumentNullException(nameof(spName));
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                var commandParameters = SqlHelperParameterCache.GetSpParameterSet(conString, spName);

                AssignParameterValues(commandParameters, parameterValues);

                return ExecuteReader(conString, CommandType.StoredProcedure, spName, commandParameters);
            }
            return ExecuteReader(conString, CommandType.StoredProcedure, spName);
        }
